...
describe Postcodesio do
  ...
  context 'requesting information on a single postcode works correctly' do
    ...
    before(:all) do
      ...
      #I FYOU WANted to generate a random postcode each time, you'd just require Faker
      #
      @single_service.get_single_postcode("GL55BP")
    end

    it "should respond with a status message of 200" do
      #If all the logic is inside the class not the test itself, it makes the test
      #more readable for humans and also helps anothe tester add more tests after you.
      expect(@single_service.get_response_code).to eq 200
    end

    it "should have a results hash" do
      expect(@single_service.get_single_postcode).to be_kind_of(Hash)
    end
